...
function detectUnsupportedClassFeatures(source: string): UnsupportedFeature[] {
  const features: UnsupportedFeature[] = [];
  for (const line of linesWithRanges(source)) {
    if (/^\s*namespace\b|^\s*(?:classDef|style|cssClass|click)\b/i.test(line.text)) {
      features.push(unsupportedSyntax(
        'class.advanced',
        line,
        'Class namespaces and styling directives are not supported yet.',
        'error',
      ));
    }
  }
  ...
  for (const line of linesWithRanges(source)) {
    ...
    if (/^(?:graticule\b|---|config:|themeVariables:|classDef\b|class\b|style\b|accTitle:|accDescr:)/i.test(trimmed) || /:::[A-Za-z0-9_-]+/.test(trimmed)) {
      features.push(unsupportedSyntax(
        'radar.advanced',
        line,
        'Radar graticules and YAML configuration are not supported yet.',
        'error',
      ));
    }
  }
...
function partialSequence(): DiagramSupportEntry {
  return {
    ...
    supportedSyntax: [
      ...
      { id: 'sequence.rect', label: 'rgb- and hex-framed sequence regions', status: 'supported' },
    ...
    unsupportedSyntax: [
      { id: 'sequence.advanced', label: 'rect frames without rgb() or hex colors and multi-line notes', status: 'unsupported' },
    ],
  };
}
...
function partialClass(): DiagramSupportEntry {
  return {
    ...
    unsupportedSyntax: [
      { id: 'class.advanced', label: 'namespaces and styling directives', status: 'unsupported' },
    ],
  };
}
...
function partialUserJourney(): DiagramSupportEntry {
  return { diagramType: 'user-journey', status: 'partial', supportedSyntax: [
    { id: 'user-journey.scored-task', label: 'sectioned tasks with 1–5 scores and actors', status: 'supported' }, { id: 'acc.accessibility', label: 'accTitle and accDescr directives surfaced as the SVG accessible name and description', status: 'supported' },
  ], unsupportedSyntax: [
    { id: 'user-journey.advanced', label: 'custom task styling', status: 'unsupported' },
  ] };
}
...
function partialGantt(): DiagramSupportEntry {
  return { diagramType: 'gantt', status: 'partial', supportedSyntax: [
    ...
    { id: 'gantt.advanced', label: 'unix dateFormat', status: 'unsupported' },
  ] };
}
function partialPie(): DiagramSupportEntry { return { diagramType: 'pie', status: 'partial', supportedSyntax: [{ id: 'pie.value', label: 'numeric labeled slices', status: 'supported' }, { id: 'pie.title', label: 'pie titles rendered above the chart', status: 'supported' }, { id: 'acc.accessibility', label: 'accTitle and accDescr directives surfaced as the SVG accessible name and description', status: 'supported' }], unsupportedSyntax: [{ id: 'pie.showData', label: 'showData directive rendering the slice value table', status: 'supported' }, { id: 'pie.advanced', label: 'custom themes', status: 'unsupported' }] }; }
...
function partialKanban(): DiagramSupportEntry { return { diagramType: 'kanban', status: 'partial', supportedSyntax: [{ id: 'kanban.columns', label: 'ordered columns with bracketed or bare labels', status: 'supported' }, { id: 'kanban.tasks', label: 'space-indented tasks within columns', status: 'supported' }, { id: 'acc.accessibility', label: 'accTitle and accDescr directives surfaced as the SVG accessible name and description', status: 'supported' }], unsupportedSyntax: [{ id: 'kanban.tickets', label: 'task `@{ ticket }` metadata rendered on cards', status: 'supported' }, { id: 'kanban.advanced', label: 'YAML configuration, ticketBaseUrl, and custom styles', status: 'unsupported' }] }; }
...
function partialRadar(): DiagramSupportEntry { return { diagramType: 'radar', status: 'partial', supportedSyntax: [{ id: 'radar.axes', label: 'three or more named axes', status: 'supported' }, { id: 'radar.curves', label: 'finite numeric curves with matching axis values', status: 'supported' }, { id: 'radar.range', label: 'title and min/max numeric range', status: 'supported' }, { id: 'acc.accessibility', label: 'accTitle and accDescr directives surfaced as the SVG accessible name and description', status: 'supported' }], unsupportedSyntax: [{ id: 'radar.advanced', label: 'graticules, YAML configuration, classes, and styles', status: 'unsupported' }] }; }
