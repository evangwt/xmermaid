use crate::types::{Dimensions, LayoutConfig, LayoutResult, PieSlice};
use xmermaid_parser::ast::PieAst;
pub fn layout(pie: &PieAst, config: &LayoutConfig) -> LayoutResult {
 let total: f64 = pie.slices.iter().map(|slice| slice.value).sum(); let mut angle = -std::f64::consts::FRAC_PI_2;
 let slices = pie.slices.iter().map(|slice| { let next = angle + slice.value / total * std::f64::consts::TAU; let result = PieSlice { label: slice.label.clone(), value: slice.value, start_angle: angle, end_angle: next }; angle = next; result }).collect();
 let title_height = if pie.title.is_empty() { 0.0 } else { 44.0 };
 let legend_width = if pie.show_data { 190.0 } else { 0.0 };
 LayoutResult { subgraph_boxes: Vec::new(), pie_show_data: pie.show_data, pie_title: (!pie.title.is_empty()).then(|| pie.title.clone()), nodes: vec![], edges: vec![], dimensions: Dimensions { width: 420.0 + config.padding * 2.0, height: 320.0 + title_height + config.padding * 2.0 }, pie_slices: slices, xy_chart: None, sankey: None, quadrant_chart: None, block_diagram: None, kanban_board: None, treemap: None, radar: None, packet: None, venn: None, swimlanes: None, sequence: None, ishikawa: None, wardley: None, cynefin: None }
}
