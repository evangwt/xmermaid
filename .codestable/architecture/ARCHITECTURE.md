...
生产默认安全策略由 `src/security.ts` 定义并通过 root API 导出 `DEFAULT_SECURITY_POLICY`、`SecurityLevel`、`SecurityPolicy`。默认 `securityLevel` 是 `strict`：不可信 Mermaid input 中的 `click` callback/link 和不在 allowlist 中的 URL protocol 会在 render preflight 阶段产生 `security_blocked_click`、`security_blocked_url` error diagnostics，并在调用 WASM 前阻断渲染。HTML label 不做 security gating：Rust parser 在解析时把标签清洗为纯文本和换行（`<br>` 变为 line break），任何 security level 下都不会把 label 当可信 HTML 渲染；`security_blocked_html` 仍是保留的 diagnostic code，但当前不会产生。默认 `sanitizeSvg` 是 `true`：`renderToSVGElement()` 在返回前清理生成 SVG DOM 中的 `script` / `foreignObject`、inline event handler 属性和危险 `href` 值。`loose` 只放宽 click 的 security blocking；危险 URL 仍然阻断。当前 allowlist 为 `http:`、`https:`、`mailto:`。v1 不执行 click callback，不把 HTML label 当 HTML 渲染，也不实现 CSP 或 sandbox。
...
- `classDef <name>` 与 `class <node-id>[,<node-id>...] <name>`：支持 `fill`、`stroke`、`color` 安全颜色值加 `stroke-width`、`stroke-dasharray`，`font-size` 等常见外观属性校验后接受并忽略（暂不渲染），多个赋值按字段级最后值覆盖；任意 CSS、URL 值仍为 unsupported，不能把跳过解析或误解析当成完整支持。
...
`DiagramBlock.range` 是安全回写合同：Markdown fence 模式下指向 fence 内部源码区域，raw Mermaid 模式下指向 trim 后源码区域，offset 使用 JS string offset 且 `endOffset` 为 exclusive。每次 `extractDiagrams()` 会为完整文本构建一次行起点索引，再通过二分查找把 offset 转为行列，避免多图文档为每张图重复扫描文本前缀。`replaceDiagramSource(text, diagramId, nextSource, document)` 是 selected source edit、repair apply 和 visual edit 共享的单图替换入口；它只替换 matched diagram 的 range，保留文档上下文，替换后重新 `extractDiagrams`，找不到 id 时返回原文并追加 `diagram_not_found` diagnostic。
...
Live editor 的 visual edit path 是异步编排：analysis 成功后应用 edit、serialize next source，再用 `validateVisualEditResult(nextSource, options?, expectedModel?)` 做 parse/render validation 和 AST-backed semantic validation；当 parsed graph model 与 expected model 的 direction、node label/shape、edge style/label/min_length 或 subgraph 语义不一致时返回 `visual_roundtrip_failed`，失败时显示 `visual_parse_failed` / `visual_roundtrip_failed` diagnostics 并保留原 source。`analyzeFlowchartForVisualEdit()` 先复用 production support analyzer `detectUnsupportedFeatures(source)` 做 safety gate；命中当前 support matrix 声明 unsupported 的 flowchart syntax，或包含尚不能无损回写的 `classDef` / `class` 声明时，返回 `read-only` + `visual_unsupported_syntax`，不构造 graph model、不执行 rewrite。因为 analysis/validation 走 async parser，`XMermaidLiveEditor` 串行化 visual edit 操作，保证连续点击 add node / add edge 时按用户操作顺序反写。
...
当前合同把 `flowchart` 标为 `partial`：基础 `graph` / `flowchart` 声明、基础节点和有向边、常见标签、核心 shape、部分 subgraph parse，以及安全的 `classDef <name>` / `class <node-id>[,<node-id>...] <name>` / `style` / `linkStyle` 属于支持范围。类定义与样式接受 `fill`、`stroke`、`color` 安全颜色值加 `stroke-width`、`stroke-dasharray`，`font-size` 等常见外观属性校验后接受并忽略（暂不渲染），多个赋值按字段级最后值覆盖；HTML label、Markdown label、quoted label、entity-code label、FontAwesome label（未知图标降级为纯文本）、edge IDs、quoted inline edge labels、inline edge labels、expanded / stadium / cylinder shape syntax、thick / extended edge forms、bidirectional / circle / cross edge endings、edges to subgraph ids、hyphenated node ids 和 inline class assignments 均在支持范围，其中标签由 parser 清洗为纯文本与换行。`click` 接受但在静态输出中无效果（warning）；任意 CSS 或 URL 值、无效 direction 和 unterminated label 仍明确列为 unsupported。`sequenceDiagram`、`classDiagram`、`stateDiagram`、`erDiagram`、`journey`、`gantt`、`pie`、`mindmap`、`timeline`、`requirementDiagram`、`gitGraph`、C4 系列和 `zenuml` 均为 partial diagram family，具体边界由 support matrix 声明；其中 `sequenceDiagram` 支持显式或从消息推断的 participant、actor 和 `as` 显示别名、带 label 的直接消息（含虚线 `--x` 交叉终止）、bare `autonumber`、activation/deactivation（含接收者 `+` 和发送者 `-` message suffix）、单行 left/right/over note、已校验的 `rect rgb(red, green, blue)` frame，以及嵌套 loop、alt/else、opt、par/and、critical/option 与 break 控制块。`create` / `destroy`、`box`、link、multi-line note 和高级 autonumber 或 rect 形式仍在 WASM 前拒绝。class diagram 支持 classDef 定义与 class 赋值着色，cssClass/click 为无操作。其余 catalog family 则在 WASM 前以 `unsupported_diagram_type` 拒绝。
...
`analyzeSupport(source)` 仍不替代 Rust parser，但现在会携带 `unsupportedFeatures`。`detectUnsupportedFeatures(source)` 是轻量 production support analyzer：unsupported diagram family 返回 `diagram.*` feature；flowchart 只对仍会破坏渲染的形式返回 `flowchart.*` feature——不安全的 `class` / `classDef` / `style` / `linkStyle` 属性值（error）、invalid direction 与 unterminated label（error）、`click`（warning，静态输出无效果）、未知 FontAwesome 图标（warning，降级为纯文本）。HTML/Markdown 标签、边 ID 与实体解码由 parser 层清洗，analyzer 不再报告。`SupportSourceRange` 使用 JS string offset，line/column 为 1-based，endOffset/endColumn 为 exclusive。Analyzer 只读 source，不调用 WASM，不修改 render path；它的 feature id 必须映射到 support matrix 的 syntax id，后续 diagnostics/runtime 只能消费这些结构化输出，不能靠字符串猜。

发布门禁现在包含 packed consumer smoke 和 docs support matrix sync。`scripts/verify-release.cjs` 的默认矩阵在 `npm run build` 之后执行 `consumer-pack-install`，命令为 `npm run --silent smoke:consumer -- --json`。该 gate 由 `scripts/consumer-smoke.cjs` 负责：在临时目录运行 `npm pack`、校验 tarball 中存在 `dist/index.d.ts`、`dist/support.d.ts`、ESM/CJS bundle、`dist/xmermaid_wasm_bg.wasm`、README、LICENSE 和 package metadata，再把 tarball 安装进临时消费者项目执行 TypeScript typecheck、root 与 `xmermaid/editor` ESM import smoke、root 与 `xmermaid/editor` CommonJS require smoke，并通过 headless Chrome 加载 installed package 的 ESM bundle，使用默认 bundle-relative WASM asset resolution 渲染最小 flowchart SVG。Chrome smoke 还通过 import map 从 `xmermaid/editor` 导入 live editor，并驱动 live editor workflow：多图切换、visual rename 反写、preview-only direction、source direction edit、unsupported visual edit blocking、share hash 和 SVG export readiness。JSON summary 记录 package size 和 browser render duration；第一阶段只记录基线，不设硬阈值。
...
当前架构只记录已经落地并由代码、测试或 release gate 证明的结构。非 flowchart 图表扩展、CLI/Server SDK、Canvas renderer、插件系统、完整 Mermaid 兼容和完整拖拽画布编辑器都不属于当前事实；这些方向如需推进，必须另起 roadmap / feature，并在真实验收后再回写本文件。

早期愿景草案仍可在 `docs/plans/` 中作为历史材料追溯，但不能作为生产合同、API 承诺或实现优先级引用。
...
5. **严格安全默认值** — strict security policy 默认阻断 risky click 和危险 URL；HTML label 由 parser 清洗为纯文本，任何 security level 都不当 trusted HTML 渲染
