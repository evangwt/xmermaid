...
- `classDef <name>` 与 `class <node-id>[,<node-id>...] <name>`：仅支持 `fill`、`stroke`、`color` 三或六位十六进制颜色，多个赋值按字段级最后值覆盖；`style`、`linkStyle`、任意 CSS、URL 值和 inline class 仍为 unsupported 或安全阻断，不能把跳过解析或误解析当成完整支持。
