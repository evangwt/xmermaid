...
`docs-support-matrix-sync` 命令为 `node scripts/verify-release.cjs --check-docs`，在默认矩阵中紧跟 `consumer-pack-install`。它读取 `README.md`、`package.json` 和 `docs/production-release-checklist.md`，检查 package description 仍声明 flowchart/partial，README 仍说明 partial Mermaid support、unsupported diagram families、diagnostics、quoted/entity-code/FontAwesome label 限制、edges to subgraph ids 限制、hyphenated node ids 限制、默认 strict security、consumer smoke、Chrome/`CHROME_BIN` 和 live editor workflow smoke，并检查 release checklist 列出默认矩阵所有 command id。该 gate 不跑 build/test，不访问网络，只防止生产承诺和机器可读支持合同漂移。
