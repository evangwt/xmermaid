...
- `class`、`classDef`、`style`、`click`、`linkStyle`、HTML label、Markdown label、entity-code label、FontAwesome label、expanded / stadium / cylinder shape syntax、thick / extended edge forms、bidirectional / circle / cross edge endings、inline edge labels、edge IDs、inline class assignments：当前 unsupported 或安全阻断；不能把跳过解析或误解析当成完整支持。
...
`docs-support-matrix-sync` 命令为 `node scripts/verify-release.cjs --check-docs`，在默认矩阵中紧跟 `consumer-pack-install`。它读取 `README.md`、`package.json` 和 `docs/production-release-checklist.md`，检查 package description 仍声明 flowchart/partial，README 仍说明 partial Mermaid support、unsupported diagram families、diagnostics、entity-code / FontAwesome label 限制、默认 strict security、consumer smoke、Chrome/`CHROME_BIN` 和 live editor workflow smoke，并检查 release checklist 列出默认矩阵所有 command id。该 gate 不跑 build/test，不访问网络，只防止生产承诺和机器可读支持合同漂移。
